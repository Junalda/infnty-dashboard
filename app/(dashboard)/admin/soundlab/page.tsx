...
export const metadata = { title: 'Sound Lab — Producer Admin — INFNTY Studio' }
...
const PLATFORMS = ['spotify', 'apple_music', 'youtube_music', 'tidal', 'amazon_music']
...
export default async function ProducerAdminPage() {
  ...
  if (adminRole !== 'producer_admin' && adminRole !== 'super_admin') {
    redirect('/admin/no-access')
  }

  const adminSupabase = await createAdminClient()

  // For producer_admin: only fetch assigned subscribers' songs
  // For super_admin: fetch all
  let songQuery = adminSupabase
    .from('song_projects')
    .select('*')
    .order('created_at', { ascending: false })

  if (adminRole === 'producer_admin') {
    // Get assigned subscriber IDs
    ...
    const assignedIds = (assignments ?? []).map((a: any) => a.subscriber_id)
    if (assignedIds.length > 0) {
      songQuery = songQuery.in('user_id', assignedIds)
    } else {
  ...
  const [
    ...
    adminSupabase
      .from('admin_assignments')
      .select('subscriber_id, profiles!admin_assignments_subscriber_id_fkey(full_name, email)')
      .eq('admin_id', user.id)
      .eq('pillar', 'soundlab'),
  ])
  ...
  const myClients = (assignments ?? []).map((a: any) => a.profiles)
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Manage song projects and production for your clients</p>
        </div>
      </div>
        ...
        <Card className="border-purple-500/20">
          ...
          <CardContent className="pt-0">
            <div className="flex flex-wrap gap-2">
              {myClients.map((c: any, i: number) => (
                <div key={i} className="px-3 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20">
                  <span className="text-xs text-purple-300">{c?.full_name ?? c?.email ?? 'Unknown'}</span>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Songs', value: allSongs.length, color: 'text-white' },
          { label: 'In Progress', value: allSongs.filter((s: any) => s.status === 'in_progress').length, color: 'text-amber-400' },
          { label: 'Delivered', value: allSongs.filter((s: any) => s.status === 'delivered').length, color: 'text-emerald-400' },
          { label: 'Live on Platforms', value: (distribution ?? []).filter((d: any) => d.status === 'live').length, color: 'text-blue-400' },
        ].map(s => (
          <Card key={s.label}>
      ...
      {allSongs.length === 0 ? (
        <Card>
          <CardContent className="p-10 text-center">
            ...
            {myClients.length === 0 && (
              <p className="text-zinc-600 text-sm mt-2">You have no assigned clients. Contact a Super Admin.</p>
            )}
          </CardContent>
        </Card>
      ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-zinc-600 mt-0.5">
                          Client: {owner?.full_name ?? owner?.email ?? 'Unknown'}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="rounded-xl bg-zinc-900 border border-zinc-800 p-4 space-y-4">
                    ...
                    <form className="space-y-3" action="/api/admin/update-song" method="post">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          <label className="text-xs text-zinc-500 mb-1 block">Production Notes</label>
                          <input
                            name="notes"
                      ...
                      {PLATFORMS.map(platform => {
                        ...
                        return (
                          <div key={platform} className="flex items-center justify-between rounded-lg bg-zinc-800 px-3 py-2">
                            <span className="text-xs text-zinc-400 capitalize">{platform.replace(/_/g, ' ')}</span>
                  ...
                  <div className="rounded-xl bg-zinc-900 border border-zinc-800 p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide">
                        Revisions ({song.revisions_used ?? 0}/{song.revisions_included ?? 0} used)
                      </p>
                    </div>
